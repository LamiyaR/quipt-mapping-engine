...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="str:split">
    ...
    <xsl:param name="skip-empty-node" select="1"/>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:call-template name="str:_split-pattern">
          ...
          <xsl:with-param name="skip-empty-node" select="$skip-empty-node"/>
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="str:_split-pattern">
    ...
    <xsl:param name="skip-empty-node"/>
    <xsl:choose>
      <xsl:when test="contains($string, $pattern)">
        <xsl:if test="not(starts-with($string, $pattern)) or $skip-empty-node!='1'">
